	...
	m_infoLogLength(0), m_result(GL_FALSE), UseGeometry(gemoetryUse), m_type(type)
{
	...
	else
	{
		...
		if ((m_type == S_PH_LIGHT) || (m_type == N_NONE))
			CreateShader("ShaderCodes\\Phong_Lighting.vs",
				"ShaderCodes\\Phong_Lighting.fs", nullptr);
		else if (m_type == S_PH_SHADING)
			CreateShader("ShaderCodes\\Phong_Shading.vs",
				"ShaderCodes\\Phong_Shading.fs", nullptr);
		else if (m_type == S_BLINN_SHADING)
			CreateShader("ShaderCodes\\Blinn_Shading.vs",
				"ShaderCodes\\Blinn_Shading.fs", nullptr);
		else if (m_type == S_AROUND)
			CreateShader("ShaderCodes\\Phong_Shading.vs",
				"ShaderCodes\\FragmentShader_Around.fs", nullptr);
		else if (m_type == S_SKYBOX)
			CreateShader("ShaderCodes\\Skybox.vs",
				"ShaderCodes\\Skybox.fs", nullptr);
		else if (m_type == S_REFLECTION)
			CreateShader("ShaderCodes\\CubeMapping.vs",
				"ShaderCodes\\CubeMapping.fs", nullptr);
	}
}
