...
int Draw_Status = 0;
int bullet_index = num_object_center;
...
int main(void)
{
	...
	for (int i = 0; i < num_object_center; ++i)
	{
		...
		object_center[i].velocity = { 0.f, 0.f, 0.f };
	}
	...
	Render object_aabb[21]; // for bullet
	for (int i = 0; i < num_object_center; ++i)
	{
	...
	Render bullet;
	bullet.position = glm::vec3(-100.f, -100.f, -100.f);
	bullet.scale = glm::vec3(1, 1, 1);
	bullet.velocity = glm::vec3(0, 0, 0);
	object_aabb[bullet_index].position = bullet.position;
	object_aabb[bullet_index].color = { 0.8f, 0.8f, 0.9f };
	object_aabb[bullet_index].scale = { 1.f, 1.f, 1.f };
	object_aabb[bullet_index].m_BVtype = Render::BoundingVolumeType::AABB;
	...
	if (!object_aabb[bullet_index].loadOBJ("models\\cube_low_poly.obj", middlePoint, bullet_index, false))
	{
		std::cout << "Failed to read object_center OBJ file!" << std::endl;
		return -1;
	}
	if (!bullet.loadOBJ("models\\sphere_high_poly.obj", middlePoint, 0, false))
	{
		std::cout << "Failed to read object_center OBJ file!" << std::endl;
		return -1;
	}
	...
	object_aabb[bullet_index].Describe(object_aabb[bullet_index].obj_vertices, object_aabb[bullet_index].obj_indices, object_aabb[bullet_index].textureUV,
		object_aabb[bullet_index].vertexNormals, false);
	bullet.Describe(bullet.obj_vertices, bullet.obj_indices, bullet.textureUV, bullet.vertexNormals, false);
	...
	object_aabb[bullet_index].position = object_center[bullet_index].position;
	object_aabb[bullet_index].scale = object_center[bullet_index].CalculateBoundingBoxScale() * 0.5f;

	bullet.aabb_box.position = bullet.position;
	bullet.aabb_box.scale = bullet.scale;
	bullet.aabb_box.xMax = object_aabb[bullet_index].xMax + object_aabb[bullet_index].position.x;
	bullet.aabb_box.xMin = object_aabb[bullet_index].xMax + object_aabb[bullet_index].position.x;
	bullet.aabb_box.yMax = object_aabb[bullet_index].yMax + object_aabb[bullet_index].position.y;
	bullet.aabb_box.yMin = object_aabb[bullet_index].yMin + object_aabb[bullet_index].position.y;
	bullet.aabb_box.zMax = object_aabb[bullet_index].zMax + object_aabb[bullet_index].position.z;
	bullet.aabb_box.zMin = object_aabb[bullet_index].zMin + object_aabb[bullet_index].position.z;

	int OcTreeOn = 0;
	BVHType m_type = TOP_DOWN;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS)
			ShootBullet(&bullet, &camera, deltaTime);
		
		shader_around.Use();
		bullet.Rendering(&camera, &shader_around, aspect, GL_TRIANGLES, bullet.position, bullet.color);

		bullet.position += bullet.velocity;
		bullet.aabb_box.position = bullet.position;
		bullet.aabb_box.xMax = object_aabb[bullet_index].xMax + bullet.position.x;
		bullet.aabb_box.xMin = object_aabb[bullet_index].xMin + bullet.position.x;
		bullet.aabb_box.yMax = object_aabb[bullet_index].yMax + bullet.position.y;
		bullet.aabb_box.yMin = object_aabb[bullet_index].yMin + bullet.position.y;
		bullet.aabb_box.zMax = object_aabb[bullet_index].zMax + bullet.position.z;
		bullet.aabb_box.zMin = object_aabb[bullet_index].zMin + bullet.position.z;

		if (m_type == TOP_DOWN)
		{
			if (DetectCollision_BroadPhase(&bullet, top_down_tree, m_type))
			{
				bullet.color = hierarchy.color;
				bullet.velocity = glm::vec3(0, 0, 0);
			}
		}
		else if (m_type == BOTTOM_UP)
		{
			if (DetectCollision_BroadPhase(&bullet, bottom_up_tree, m_type))
			{
				bullet.color = hierarchy.color;
				bullet.velocity = glm::vec3(0, 0, 0);
			}
		...
		{
			if (DetectCollision_BroadPhase(&bullet, m_ocTree))
				bullet.velocity = glm::vec3(0, 0, 0);
		}
