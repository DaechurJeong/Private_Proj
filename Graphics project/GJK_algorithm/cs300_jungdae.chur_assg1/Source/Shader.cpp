	...
	m_infoLogLength(0), m_result(GL_FALSE), UseGeometry(gemoetryUse), m_type(type)
{
	...
	else
	{
		...
		else if (m_type == N_BOUNDING_BOX)
			CreateShader("ShaderCodes\\VertexShader_BoundingBox.vs",
				"ShaderCodes\\FragmentShader_BoundingBox.fs", nullptr);
		else if (m_type == S_OCTREE)
			CreateShader("ShaderCodes\\OcTree.vs",
				"ShaderCodes\\OcTree.fs", nullptr);
	}
}
