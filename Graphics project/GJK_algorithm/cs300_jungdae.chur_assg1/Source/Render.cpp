...
#define EPA_MAX_NUM_ITERATIONS 20
...
glm::vec3 Collider::support(glm::vec3 dir)
{
	...
	vertices[0] = glm::vec3(xMin, yMin, zMin);
	vertices[1] = glm::vec3(xMin, yMin, zMax);
	vertices[2] = glm::vec3(xMax, yMin, zMax);
	vertices[3] = glm::vec3(xMax, yMin, zMin);

	vertices[4] = glm::vec3(xMax, yMax, zMax);
	vertices[5] = glm::vec3(xMax, yMax, zMin);
	vertices[6] = glm::vec3(xMin, yMax, zMin);
	vertices[7] = glm::vec3(xMin, yMax, zMax);

	float f_max = std::numeric_limits<float>::min();
	int index = 0;
	for (int i = 0; i < vertices.size(); ++i)
	{
		float dot = glm::dot(vertices[i], dir);
		if (dot > f_max)
		{
			f_max = dot;
			index = i;
	...
	return glm::vec3(vertices[index]);
	/*const static glm::vec3 up(0, 1, 0);
	glm::mat4 identity_scale(1.0);
	glm::mat4 identity_rotation(1.0);
	glm::mat4 matRS = glm::scale(identity_scale, scale);
	glm::mat3 mat_RS = matRS;

	dir = glm::inverse(mat_RS) * dir;

	glm::vec3 result;
	result.x = (dir.x > 0) ? xMax : xMin;
	result.y = (dir.y > 0) ? yMax : yMin;
	result.z = (dir.z > 0) ? zMax : zMin;
	return mat_RS * result + position;*/
}
...
void ShootBullet(Render* obj, Camera* cam, float dt)
{
	obj->position = cam->position;
	obj->velocity = 2.f * (cam->front * dt);
	obj->aabb_box.position = obj->position;
}
bool BBOX_intersection(Collider box1, Collider box2)
{
...
bool gjk(Collider coll1, Collider coll2) {
	...
	glm::vec3 search_dir = coll1.position - coll2.position; //initial search direction between colliders
	...
	if (glm::dot(b, search_dir) < 0) { return false; }//we didn't reach the origin, won't enclose it

	search_dir = glm::cross(glm::cross(c - b, -b), c - b); //search perpendicular to line segment towards origin
	...
	for (int iterations = 0; iterations < GJK_MAX_NUM_ITERATIONS; iterations++)
	{
		...
		if (glm::dot(a, search_dir) < 0) { return false; }//we didn't reach the origin, won't enclose it
...
void update_simplex3(glm::vec3 &a, glm::vec3 &b, glm::vec3 &c, glm::vec3 &d, int &simp_dim, glm::vec3 &search_dir) 
{
	...
	if (glm::dot(glm::cross(n, c - a), AO) > 0) { //Closest to edge AC
		b = a;
		search_dir = glm::cross(cross(c - a, AO), c - a);
		return;
	}
...
bool DetectCollision_BroadPhase(Render* obj, HierarchyManager* RootNode, BVHType type)
{
	Collider worldBox = RootNode->m_collider;
	Collider sphereBox = obj->aabb_box;
	if (!BBOX_intersection(worldBox, sphereBox))
		return false;
	if (RootNode->right_)
	{
		if (DetectCollision_MidPhase(obj, RootNode->right_, type))
			return true;
	}
	if (RootNode->left_)
	{
		if (DetectCollision_MidPhase(obj, RootNode->left_, type))
			return true;
	}
	return false;
}
bool DetectCollision_MidPhase(Render* obj, HierarchyManager* treeNode, BVHType type)
{
	if (treeNode == NULL)
		return false;
	if (treeNode->node_type == LEAF)
	{
		if (gjk(obj->aabb_box, treeNode->m_collider))
		{
			//Render polygon of treeNode & sphere in Red color
			if (type == TOP_DOWN)
			{
				obj->color = glm::vec3(1.f, 0.f, 0.f);
				treeNode->color = glm::vec3(1.f, 0.f, 0.f);
			}
			else if (type == BOTTOM_UP)
			{
				obj->color = glm::vec3(0.f, 0.f, 1.f);
				treeNode->color = glm::vec3(0.f, 0.f, 1.f);
			}
			obj->velocity = glm::vec3(0, 0, 0);
		}
	}
	Collider nodeBox = treeNode->m_collider;
	Collider sphereBox = obj->aabb_box;
	if (!BBOX_intersection(nodeBox, sphereBox))
		return false;
	if (treeNode->right_)
	{
		if (DetectCollision_MidPhase(obj, treeNode->right_, type))
			return true;
	}
	if (treeNode->left_)
	{
		if (DetectCollision_MidPhase(obj, treeNode->left_, type))
			return true;
	}
	return false;
}
bool DetectCollision_BroadPhase(Render* obj, OctreeNode* RootNode)
{
	Collider worldBox = RootNode->m_collider;
	Collider sphereBox = obj->aabb_box;
	if (!BBOX_intersection(worldBox, sphereBox))
	...
	for (int i = 0; i < 8; ++i)
	{
		if (DetectCollision_MidPhase(obj, RootNode->pChild[i]))
			return true;
	}
	return false;
}
bool DetectCollision_MidPhase(Render* obj, OctreeNode* treeNode)
{
	if (!treeNode)
		return false;
	if (treeNode->node_type == LEAF)
	{
		if (gjk(obj->aabb_box, treeNode->m_collider))
		{
			//Render polygon of treeNode & sphere in Red color
			obj->color = glm::vec3(1.f,0.f,0.f);
			treeNode->box_color = glm::vec3(1.f, 0.f, 0.f);
			obj->velocity = glm::vec3(0, 0, 0);
		}
	}
	Collider nodeBox = treeNode->m_collider;
	...
	for (int i = 0; i < 8; ++i)
	{
		if (DetectCollision_MidPhase(obj, treeNode->pChild[i]))
			return true;
	}
	return false;
}
void TopDownBVTree(HierarchyManager** tree, std::vector<glm::vec3> point_, int depth_, ShapeType shape_)
{
	...
	*tree = pNode;
	glm::vec3 max_axis = ComputeBoundingVolume(point_, pNode->BVposition, pNode->BVscale, shape_, pNode->m_collider);
	pNode->depth = depth_;
	pNode->color = glm::vec3(1.f, 1.f, 1.f);

	if (point_.size() <= (size_t)MIN_POINTS_PER_LEAF)
	{
		pNode->node_type = LEAF;
		pNode->points = point_;
	}
	else
	{
		pNode->node_type = NODE;
...
HierarchyManager* BottomUpBVTree(std::vector<Render*> obj, ShapeType shape_)
{
	...
	for (int k = 0; k < num_obj; ++k)
	{
		...
		pNodes[k]->depth = 1;
		pNodes[k]->node_type = LEAF;
		pNodes[k]->m_collider.position = obj[k]->position;
		pNodes[k]->m_collider.scale = obj[k]->scale;
		pNodes[k]->m_collider.xMax = obj[k]->xMax + obj[k]->position.x;
		pNodes[k]->m_collider.xMin = obj[k]->xMin + obj[k]->position.x;
		pNodes[k]->m_collider.yMax = obj[k]->yMax + obj[k]->position.y;
		pNodes[k]->m_collider.yMin = obj[k]->yMin + obj[k]->position.y;
		pNodes[k]->m_collider.zMax = obj[k]->zMax + obj[k]->position.z;
		pNodes[k]->m_collider.zMin = obj[k]->zMin + obj[k]->position.z;
		pNodes[k]->color = glm::vec3(1.f, 1.f, 1.f);
	}
	while (num_obj > 1) {
		...
		ComputeBoundingVolume_BottomUp(pNodes[i], pNodes[j],
			pPair->BVposition, pPair->BVscale, shape_, pPair->m_collider);

		pPair->depth = pNodes[i]->depth + 1;
		pPair->node_type = NODE;
		pPair->m_collider.position = pPair->BVposition;
		pPair->m_collider.scale = pPair->BVscale;
		pPair->color = glm::vec3(1.f, 1.f, 1.f);
		int min = i, max = j;
		if (i > j)
			min = j, max = i;
		pNodes[min] = pPair;
...
OctreeNode* BuildOctree(std::vector<Render*> obj, glm::vec3 center, float halfWidth_, int depth_, glm::vec3 color_)
{
	...
	pNode->depth = depth_;
	pNode->box_color = color_;
	pNode->m_collider.position = pNode->center;
	pNode->m_collider.scale = glm::vec3(halfWidth_, halfWidth_, halfWidth_);
	pNode->m_collider.xMax = center.x + halfWidth_;
	pNode->m_collider.xMin = center.x - halfWidth_;
	pNode->m_collider.yMax = center.y + halfWidth_;
	pNode->m_collider.yMin = center.y - halfWidth_;
	pNode->m_collider.zMax = center.z + halfWidth_;
	pNode->m_collider.zMin = center.z - halfWidth_;
	if (num_triangle < 300)
	{
		if (num_triangle == 0)
			return nullptr;
	...
	Collider& m_col)
{
	...
	glm::vec3 all_max = glm::max(obj_1_max, obj_2_max);
	glm::vec3 all_min = glm::min(obj_1_min, obj_2_min);
	m_col.xMax = all_max.x;
	m_col.yMax = all_max.y;
	m_col.zMax = all_max.z;
	m_col.xMin = all_min.x;
	m_col.yMin = all_min.y;
	m_col.zMin = all_min.z;
	...
	Collider& node_col)
{
	...
	if (points_.empty())
		return glm::vec3(0);
	float very_min_x = 100, very_max_x = -100, // default -100 to 100
		very_min_y = 100, very_max_y = -100,
		very_min_z = 100, very_max_z = -100;
	...
	node_col.position = position_;
	node_col.scale = scale_;
	node_col.xMax = very_max_x;
	node_col.xMin = very_min_x;
	node_col.yMax = very_max_y;
	node_col.yMin = very_min_y;
	node_col.zMax = very_max_z;
	node_col.zMin = very_min_z;

	return longest_axis;
}
glm::vec3 Render::support(glm::vec3 dir)
{
	const static glm::vec3 up(0, 1, 0);
	glm::mat4 identity_scale(1.0);
	glm::mat4 identity_rotation(1.0);
	glm::mat4 matRS = glm::scale(identity_scale, scale) * glm::rotate(identity_rotation, rotation, up);
	glm::mat3 mat_RS = matRS;
	dir = glm::inverse(mat_RS) * dir;
...
void Render::RenderBoundingBoxHierarchy(Camera* camera, Shader* shader, float aspect, glm::vec3 pos, glm::vec3 color_, HierarchyManager* tree, int currDepth)
{
	if (tree->depth == currDepth)
	{
		...
		shader->SetVec3("color", tree->color);
