...
void ComputeBoundingVolume_BottomUp(HierarchyManager* obj_1, HierarchyManager* obj_2, glm::vec3& bv_position, glm::vec3& bv_scale, ShapeType shape_, Collider& m_col);
float MergeTwoBoundingBox(HierarchyManager* obj_1, HierarchyManager* obj_2); // return distance

glm::vec3 ComputeBoundingVolume(std::vector<glm::vec3>& points_, glm::vec3& position_, glm::vec3& scale_, ShapeType shape_, Collider& node_col);
...
public:
	Render();
	~Render();
	Collider getBBox() { return aabb_box; };
	glm::vec3 support(glm::vec3 dir);
	...
	Collider aabb_box;
	glm::vec3 position, scale, color, velocity;
