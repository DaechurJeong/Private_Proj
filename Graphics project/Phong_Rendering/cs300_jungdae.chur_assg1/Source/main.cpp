...
int main(void)
{
	...
	object_center.color = { 0.6f, 0.1f, 0.3f };
	object_center.scale = { 0.8f, 0.8f, 0.8f };
	...
	{
		pos[i].x = 2 * sinf(i * glm::radians(theta));
		pos[i].z = 2 * cosf(i * glm::radians(theta));
	}
	/* Orbit trace */
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < Defaultlight.num_spheres; ++i)
		{
			if (isMove % 2)
			{
				pos[i].x = 2 * sinf(i * glm::radians(theta) + angle);
				pos[i].z = 2 * cosf(i * glm::radians(theta) + angle);
