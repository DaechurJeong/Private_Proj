...
public:
	...
	void Update(Shader* shader)
	{
		for (int i = 0; i < 16; ++i)
		{
			...
			// for volume of light limit
			float maxBrightness = std::fmaxf(std::fmaxf(color[i].x, color[i].y), color[i].z);
			radius[i] = (-linear + std::sqrt(linear * linear - 4 * quadratic * (constant - (256.0f / 5.0f) * maxBrightness))) / (2.0f * quadratic);
			if (m_type[i] == N_DIRECTIONAL)
			{
				...
				shader->SetFloat("dirLight[" + number + "].radius", radius[i]);
			}
			if (m_type[i] == N_POINT)
			{
				...
				shader->SetFloat("pointLight[" + number + "].radius", radius[i]);
			}
			if (m_type[i] == N_SPOT)
			{
				...
				shader->SetFloat("spotLight[" + number + "].radius", radius[i]);
			}
